...
def scan_file():
    ...
    url_directory = r"images"
    zip_directory = r"Zip_images"
    try:
        try:
            print("delete images folder")
    ...
    if request.method == 'POST':
        try:
            ...
            if request.form['text'] != '':
                ...
                if not os.path.exists(r"Zip_images"):
                    os.mkdir(r"Zip_images")
                zipfile_db_fs.put(image_data, filename=image_data.filename)
                with zipfile.ZipFile(image_data, 'r') as zip_ref:
                    zip_ref.extractall(r"Zip_images")
                file_names_folder =  os.path.join(os.path.dirname(__file__), "Zip_images")
                print(file_names_folder)
                file_names = []
                ...
                if file_name.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
                    ...
                    try:
                        ...
                        if len(mtcnn_loc) == 0:
                            ...
                            meta = {
                                'name': "MTCNN_no_output_detected_{0}.{1}".format(
                                file_name_split, image_extension),
                                'images': [
                                    {
                                        'imageID': imageID,
                    ...
                    try:
                        if "Unknown" in face_names:
                            ...
                            if len(dlib_loc) == 0:
                                ...
                                # create our image meta data
                                meta = {
                                    'name': "Dlib_no_output_detected_{0}.{1}".format(file_name_split, image_extension),
                                    'images': [
                                        {
                                            'imageID': imageID,
                                            'shape': image.shape,
                                ...
                                for face_encoding in face_encodings:
                                    ...
                                    for (top, right, bottom, left), name in zip(dlib_loc, face_names):
                                        ...
                                        # create our image meta data
                                        meta = {
                                            'name': "Dlib_{0}_{1}.{2}}".format(file_name_split, name, image_extension),
                                            'images': [
                                                {
                                                    'imageID': imageID,
                                                    'shape': image.shape,
                    ...
                    try:
                        if "Unknown" in face_names:
                            ...
                            if len(face_locations) == 0:
                                ...
                                # create our image meta data
                                meta = {
                                    'name': "Face_recog_no_output_detected_{0}.{1}".format(
                                    file_name_split, image_extension),
                                    'images': [
                                        {
                                            'imageID': imageID,
                                ...
                                for face_encoding in face_encodings:
                                    ...
                                    for (top, right, bottom, left), name in zip(dlib_loc, face_names):
                                        ...
                                        # create our image meta data
                                        meta = {
                                            'name': "Face_recog_{0}_{1}.{2}".format(file_name_split,  name, image_extension),
                                            'images': [
                                                {
                                                    'imageID': imageID,
                                                    'shape': image.shape,
