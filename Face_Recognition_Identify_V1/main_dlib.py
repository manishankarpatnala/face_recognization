    ...
    if request.method == 'POST':
        try:
            ...
            if len(url_data) != 0:
                ...
                zip_file = os.path.dirname(__file__) + r"\images.zip"
                # zip_file1, zip_file2 = os.path.split(zip_file)
                # folder_path = os.path.join(zip_file1, "images")
                ...
                if file_name.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
                    ...
                    try:
                        ...
                        if len(face_locations) == 0:
                            file_path = os.path.join(save_folder_path, "{0}\Face_recog_no_output_detected.{1}".format(os.path.split(file_name)[1][:-6], extension))
                        ...
                        if not "Unknown" in face_names:
                                for (top, right, bottom, left), name in zip(dlib_loc, face_names):
                                    ...
                                    file_path = os.path.join(save_folder_path, "{0}\Face_recog_{1}.{2}".format(os.path.split(file_name)[1][:-6], name, extension))
                                    cv2.imwrite(file_path, small_frame)

                    except Exception as e:
                        print(f"MTCNN_ error : {Exception}")
                    ...
                    try:
                        if "Unknown" in face_names:
                            ...
                            if len(dlib_loc) == 0:
                                file_path = os.path.join(save_folder_path, "{0}\Dlib_no_output_detected.{1}".format(os.path.split(file_name)[1][:-6], extension))
                                shutil.copy(file_name, file_path)
                            else:
                            ...
                            if not "Unknown" in face_names:
                                    for (top, right, bottom, left), name in zip(dlib_loc, face_names):
                                        ...
                                        font = cv2.FONT_HERSHEY_SIMPLEX
                                        cv2.putText(small_frame, name, (left + 1, bottom + 10), font, 0.6, (255, 225, 255), 2)
                                        file_path = os.path.join(save_folder_path, "{0}\Dlib_{1}.{2}".format(os.path.split(file_name)[1][:-6], name, extension))
                                        cv2.imwrite(file_path, small_frame)
                    ...
                    try:
                        if "Unknown" in face_names:
                            ...
                            if len(mtcnn_loc) == 0:
                                file_path = os.path.join(save_folder_path, "{0}\MTCNN_no_output_detected.{1}".format(os.path.split(file_name)[1][:-6], extension))
                                shutil.copy(file_name, file_path)
                                ...
                                for face_encoding in face_encodings:
                                    ...
                                    for (top, right, bottom, left), name in zip(mtcnn_loc, face_names):
                                        ...
                                        font = cv2.FONT_HERSHEY_SIMPLEX
                                        cv2.putText(small_frame, name, (left + 1, bottom + 10), font, 0.6, (255, 225, 255), 2)
                                        file_path = os.path.join(save_folder_path, "{0}\MTCNN_{1}.{2}".format(os.path.split(file_name)[1][:-6], name, extension))
                                        cv2.imwrite(file_path, small_frame)
                    except Exception as e:
